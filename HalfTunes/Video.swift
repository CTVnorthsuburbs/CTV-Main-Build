...
open class Video: NSObject, NSCoding
{
    ...
    init?(title: String, thumbnail: UIImage?,fileName: Int?, sourceUrl: String?, comments: String, eventDate: Date, thumbnailUrl: NSURL?, id: Int?, isEvent: Bool?, endDate: Date?)
    {
        ...
        let str = title
        var newString = ""
        let suffix = String(describing: str.suffix(6))
        if suffix.contains("-")
        {
        ...
        if(newString != "") {self.title = (newString)}
    ...
    func setEndDate(date: Date) {self.endDate = date}
    func getStartDate() -> Date? {return self.eventDate}
    func getEndDate() -> Date? {return self.endDate}
    func getIsEvent() -> Bool
    {
        if(self.isEvent != nil) {return self.isEvent!}
        else {return false}
    }
    
    func hasThumbnailUrl() -> Bool
    {
        if(self.thumbnailUrl != nil) {return true}
        else {return false}
    }
    ...
    open func generateThumbnail()
    {
        ...
        do
        {
            ...
            // lay out his image view, or if it already exists, set its image property to uiImage
        }
