...
class CategoriesTableViewController: UITableViewController
{
    ...
    @IBAction func cancelButtonPressed(_ sender: Any) {self.dismiss(animated: true, completion: {})}
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> CategoriesTableViewCell
    {
        ...
        {
            if( categoriesVideos[indexPath.row].fileName == 1) {thumbnail = search.getThumbnail(url: (categoriesVideos[indexPath.row].thumbnailUrl)!)}
            else if ( categoriesVideos[indexPath.row].fileName == 0){thumbnail = categoriesVideos[indexPath.row].thumbnail}
            else{if(categoriesVideos[indexPath.row].fileName != nil) {thumbnail = search.getThumbnail(id: (categoriesVideos[indexPath.row].fileName)!)}}
        }
        else
        {
        ...
        if(thumbnail != nil ) {cell?.thumbnailImage.image = thumbnail}
        else { cell?.thumbnailImage.image = #imageLiteral(resourceName: "defaultPhoto")}
